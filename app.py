...
if hist_data.empty:
    ...
    vol_colors = ['red' if close >= open else 'green' for close, open in zip(hist_data['Close'], hist_data['Open'])] # 台股收紅為上漲(紅)，下跌為綠
    ...
    hist_colors = ['red' if val >= 0 else 'green' for val in hist_data['Histogram']] # 配合台股紅漲綠跌
        ...
        if ceo_rss_url:
            ...
            try:
                ...
                if ceo_feed.entries:
                    ...
                    if ceo_texts:
                        fb_intel_text += "\n\n【官方社群動態】：\n" + "\n".join(ceo_texts) 
                        st.success(f"➤ 成功攔截 {len(ceo_texts)} 篇官方近期動態！")
                        with st.expander("👀 點擊查看官方發文標題"):
                            for t in ceo_titles:
                                st.markdown(f"- {t}")
                    else:
                        st.info("近期暫無官方動態。")
